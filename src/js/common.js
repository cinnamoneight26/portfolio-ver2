...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function experienceFilter() {
    ...
    if (contentElement) {
      contentElement.style.maxHeight = "".concat(
  ...
  function initializeCheckboxes() {
    var all = expCheckboxes.all,
      dev = expCheckboxes.dev,
      etc = expCheckboxes.etc;
    ...
    experienceItems.forEach(function (item) {
      ...
      if (months !== null && dataType) {
        var label = document.querySelector(
          "label[for='exp".concat(
            dataType.charAt(0).toUpperCase() + dataType.slice(1),
            "']"
          )
        );
        if (label) label.innerText += " (".concat(months, " months)");
      }
    });
  }
  // 리사이징 이벤트 처리 함수
  function handleResize() {
    console.log("리사이즈 발생"); // 디버그용
    initializeSections();
  }
  // 리사이즈 이벤트 리스너 추가
